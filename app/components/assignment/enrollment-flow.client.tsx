 ...
 *
 * MUST be .client.tsx because it uses useTransaction (which imports useWallet).
 *
 * @see ~/hooks/tx/use-transaction.ts — Transaction lifecycle
 * @see ~/hooks/api/course/use-assignment-commitment.ts — Commitment data
 * @see ~/components/assignment/evidence-form.tsx — Form component
 */
...
} from "~/components/assignment/evidence-form";
...
export function EnrollmentFlow({
  ...
  const handleSubmit = useCallback(
    ...
    [jwt, courseId, moduleCode, sltHash, alias, isUpdate, execute, invalidateCommitment]
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <EvidenceForm
        ...
        submitLabel={
          isUpdate ? "Update & Resubmit" : "Enroll & Submit"
        }
      />
    </div>
  );
