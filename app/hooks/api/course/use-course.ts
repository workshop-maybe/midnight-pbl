/**
 * Course & Module types, transforms, and React Query hooks
 *
 * Architecture: Colocated Types Pattern
 * - App-level types (CourseModule, SLT, Lesson, etc.) defined here with camelCase fields
 * - Transform functions convert API snake_case to app camelCase
 * - Components import types from this hook, never from raw API types
 * - Transforms are exported for server-side reuse in gateway.server.ts
 *
 * @see ~/projects/01-projects/cardano-xp/src/hooks/api/course/use-course.ts
 * @see ~/projects/01-projects/cardano-xp/src/hooks/api/course/use-course-module.ts
 */
